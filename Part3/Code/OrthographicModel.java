...
public class OrthographicModel {
    ...
    public void add( OrthographicModel model, int xShift, int yShift, int zShift ){
        for( GeometricPrimitive p : model.getModel() ){
            p.translate( xShift, yShift, zShift );
            this.add ( p );
        }
    }

    public ArrayList<GeometricPrimitive> getModel(){
        return model;
    }
    ...
    public String toString(){
        String s = "Model:\n";
        for( GeometricPrimitive p : model ){
            s += p.toString();
        }
